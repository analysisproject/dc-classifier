...
with col3:
    ...
    try:
        if roof_result is not None:
            ...
            if wide_result is not None:
                st.write(
                    f"이번 최종 판정은 **roof view 단일 결과**를 기준으로 도출했습니다. "
                    f"그 이유는 roof view가 wide view보다 대상 건물의 **지붕 형태, 건물 배치, 냉각 설비로 추정될 수 있는 구조물, "
                    f"대형 설비가 놓인 평면적 특성**을 더 직접적으로 보여주기 때문입니다. "
                    f"반면 wide view는 대상 건물 자체보다 **주변 도로, 인접 건물, 주변 산업단지 또는 상업지 맥락**이 더 많이 포함되므로, "
                    f"보조적인 참고 정보로 활용했습니다.\n\n"
                    f"roof view 분석 결과는 **{final_label}** 이며, "
                    f"모델이 계산한 데이터센터 확률은 **{final_prob:.4f}** 입니다. "
                    f"또한 roof 기준 내부 판정값은 **{roof_score_text}** 로 나타났습니다. "
                    f"이 값은 현재 선택한 분류 모드가 무엇인지에 따라 의미가 달라집니다. "
                    f"zeroshot 또는 centroid 모드라면, 이 값은 데이터센터 쪽 특징과 비데이터센터 쪽 특징 사이의 **유사도 차이(margin)** 를 뜻하고, "
                    f"linearprobe 모드라면 모델이 직접 추정한 **양성 클래스 확률**에 해당합니다.\n\n"
                    f"한편 wide view 분석 결과는 **{wide_label}** 이며, "
                    f"wide 기준 데이터센터 확률은 **{wide_prob:.4f}**, "
                    f"내부 판정값은 **{wide_score_text}** 입니다. "
                    f"즉, wide view는 주변 맥락 차원에서 현재 위치가 데이터센터로 보이는지, "
                    f"혹은 물류시설·공장·업무시설처럼 보이는지에 대한 추가 단서를 제공합니다.\n\n"
                    f"따라서 이 화면에서는 **roof view를 최종 의사결정 기준**으로 삼고, "
                    f"**wide view는 그 판정을 보강하거나 해석을 넓혀주는 참고 정보**로 이해하면 됩니다. "
                    f"만약 roof와 wide 결과가 서로 다르게 나온다면, "
                    f"그 경우는 대상 건물 자체의 형태는 데이터센터와 유사하지만 주변 입지 맥락은 그렇지 않거나, "
                    f"반대로 주변 맥락은 산업·설비형 시설에 가깝지만 지붕 구조만으로는 데이터센터 특징이 약한 경우일 수 있습니다."
                )
            else:
                st.write(
                    f"이번 최종 판정은 **roof view 단일 결과**를 기준으로 도출했습니다. "
                    f"현재 roof view 분석 결과는 **{final_label}** 이며, "
                    f"모델이 계산한 데이터센터 확률은 **{final_prob:.4f}** 입니다. "
                    f"roof 기준 내부 판정값은 **{roof_score_text}** 입니다.\n\n"
                    f"이 판정은 대상 위치의 지붕 형태와 건물의 평면적 배치에 기반해 이루어졌습니다. "
                    f"즉, 모델은 roof 이미지에서 데이터센터와 유사한 패턴이 더 강한지, "
                    f"혹은 물류창고·공장·업무용 건물처럼 보이는 특징이 더 강한지를 비교하여 결과를 냈습니다. "
                    f"현재 사용 중인 분류 모드가 zeroshot 또는 centroid라면, "
                    f"이 내부 판정값은 데이터센터 특징과 비데이터센터 특징 사이의 **유사도 차이(margin)** 를 의미하고, "
                    f"linearprobe라면 모델이 직접 계산한 **데이터센터 예측 확률**에 해당합니다.\n\n"
                    f"즉, 이 결과는 단순히 하나의 라벨만 출력한 것이 아니라, "
                    f"지붕 구조가 데이터센터형 패턴에 얼마나 가까운지를 수치적으로 비교한 뒤 도출된 판정이라고 이해하면 됩니다."
                )
